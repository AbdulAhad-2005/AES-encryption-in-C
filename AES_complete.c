...
int main() {
	...
	puts("\nEnter the text you want to encrypt using AES (The text should not contain a newline character): ");    // prompting the user to enter text to encrypt
    ...
    // /* printing the text user entered in the hexadecimal format */
    // for (int i = 0; i < plaintextLen; i++)
    //     printf("%02x ", plaintext[i]);
    puts("");

    unsigned char key[AES_block_size + 1];    // +1 for null character
    printf("Enter 16 digit secret key to encrypt your text: ");   
    scanf("%16s", key);
    ...
    puts("");
    puts("In plaintext form: ");
    for (int i = 0; i < ciphertextLen; i++)
        printf("%c", ciphertext[i]);
    puts("\n");

    unsigned char *decryptedText = (unsigned char*)malloc(plaintextLen + 1);    // +1 for null character    
    ...
    ECB_decrypt(ciphertext, expandedKey, decryptedText, plaintextLen);
    decryptedText[plaintextLen] = '\0';
    puts("Decrypted text:");
    puts("In hexadecimal form: ");
    for (int i = 0; i < plaintextLen; i++)
